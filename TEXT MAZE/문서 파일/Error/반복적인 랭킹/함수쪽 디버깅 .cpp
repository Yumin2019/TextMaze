...
void SetMaze()
{
	...
	*/
	...
	if (cnt == 5 || stage == -1) return; // cnt == 5 인 경우는 title을 출력하는 경우로 아래를 출력하지 않아도 된다. 
						 // title을 출력할 때 이후에 출력할 필요가 없다.
...
void title() // 타이틀은 stage = -1으로 그림 그리기 위한 설정이다.
{
	...
	while (true)
	{
		...
		if (_kbhit())
		{
			...
			case ENTER:
				...
				case 3: //랭킹 ( 파일 입출력을 통한 offline 랭킹 )
					...
					titleSpace(); // title 그림에 TextMaze그림이 없음
		...
		case 4:
			...
			title(); // Setmaze(랭킹보고 title) -> stage 설정 이후에 -> setMaze loop ( 근데 왜 튜토리얼로 ? )
			break;
			}
			con = 1;
			return;
...
void rankingLoop() // cnt == 5 일 때 끊어준 loop에 대한 처리
{
	Fmod->update();
	cnt = 0;
	system("cls");
...
void ranking()
{
	...
	cnt += 1; // 5번에서 다시 호출하면 loop를 끊고 다시 처리하도록 설정.
...
void stageSelect()
{
	...
	while (true)
	{
		...
		if (_kbhit())
		{
			...
			SetMaze(); // 자체 내부 cls를 막아야 옵션이 뜬다. locate == 0  rankingLoop와는 무관 ( 이곳은 스테이지 선택이라서 )
